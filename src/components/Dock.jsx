...
import { openWindow } from "../store/feature/windowSlice";
...
const Dock = () => {
  ...
  return (
    <div className="bg-white/40 backdrop-blur-xl border border-white/15 h-16 rounded-2xl absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center justify-center gap-3 px-2 py-1 ">
      {dockApps.map((app) => (
        <div key={app.id} className="relative">
          <img
            onClick={() => dispatch(openWindow(app.id))}
            src={app.icon}
            alt={app.alt}
            className="w-12 h-12 cursor-pointer hover:scale-110 transition duration-200"
          />
          {windows[app.id] && <span className="inline-block w-1 h-1 bg-gray-800 rounded-full absolute left-1/2 -translate-x-1/2 -bottom-1"></span>}
        </div>
      ))}
    </div>
  );
